...
namespace CrystalSync.Class
{
    class MysqlConnectionClass
    {
        ...
        //
        private string connectiondetails = MysqlConnectionServerSettings.Default["dbServer"].ToString();
        ...
        public MysqlConnectionClass(string conn)
        ...
        private void Connect()
        {
            try
            {
                MySqlConnection connection = new MySqlConnection(this.connString);
                connection.Open();
                connect = connection;
                bConnected = true;
                isConnected = true;
        ...
        private void Init(string Query, string[] bindings = null)
        {
            ...
            using (command = new MySqlCommand(Query, connect))
            {
                ...
                if (squery.Contains("delete") || squery.Contains("update") || squery.Contains("insert") || squery.Contains("truncat"))
                {
        ...
        //    string myConnectionString = String.Format("server={0}; uid={1}; Port={2}; pwd={3}; database={4}; Character Set=utf8;", dbServer, dbuserName, dbPort, dbPassword, dbName);
